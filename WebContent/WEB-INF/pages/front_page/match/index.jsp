...
<script type="text/javascript">
	...
	$(function() {
		...
		reflushData();
		setInterval(reflushBasketballData, 60000)
		reflushBasketballData();
	...
	function reflushBasketballData() {
		...
		for (var i = 0; i < 3; i++) {
			var parseData = "";
			if (i == 0 && data1.length > 100) {
				parseData = data1;
			} else if (i == 1 && data2.length > 100) {
				parseData = data2;
			} else if(i == 2 && data3.length > 100) {
				parseData = data3;
			}
		...
		$(".selectItemForBasketball").click(function() {
			...
			if (bet_table_html.indexOf(id) != -1) { //已存在，就删除选中
				...
				selected += '<td style="width: 13%">' + options[5] + '</td>';
				selected += '<td style="width: 13%">' + options[6] + '</td>';
				selected += '<td style="width: 240px"  class="td_multipleForBasketball"><button onclick="minusNumForBasketball(this)">-</button>&nbsp;&nbsp;<input readonly="readonly" type="text" style="width: 80px" value="1" data-option="' + option + '" id="' + options[0] + '_' + options[6] + '_multiple">&nbsp;&nbsp;<button onclick="plusNumForBasketball(this)">+</button></td>';
			...
			if("series" == selectedTypeValueForBasketball) {
				$("#rad_singleForBasketball").click();
				$("#rad_seriesForBasketball").click();
			}
	...
	function cacularTotalCountForBasketball() { //计算总额
		...
		if("single" == selectedTypeValueForBasketball) {
			...
			$("#totalCountForBasketball").val(parseInt(selectLength) * parseInt(series_multiple) * 2);
		}
	}
	...
	function betSubmitForBasketball() {
		...
		$("#txt_totalCountForBasketball").val(totalCount);
...
<body>
			...
			<div style="border: 1px solid blue; width: 100%; min-height: 300px; overflow: auto;" id="bet_div">
				...
				<table border="1px" style="width: 100%; text-align: center;" class="table" id="bet_tableForBasketball">
					<tr>
						...
						<td style="width: 13%">胜负</td>
						<td style="width: 240px;" class="td_multipleForBasketball">倍数</td>
						<td style="width: 5%">操作</td>
					</tr>
				</table>
			</div>
	...
	<form action="${pageContext.request.contextPath}/subbasketbet.url" style="display: none" method="post" id="bet_form2ForBasketball">
		<input type="text" id="txt_bet_dataForBasketball" name="data"> <input type="text" id="txt_totalCountForBasketball" name="totalCount">
		<input type="text" id="txt_typeForBasketball" name="type">
	</form>
</body>
</html>