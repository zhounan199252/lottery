	...
	$(function() {
		...
		$(".selectItem").click(function() {
			...
			if(bet_table_html.indexOf(id) != -1) { //已存在，就删除选中
				...
				selected += '<td style="width: 15%"><button onclick="minusNum(this)">-</button>&nbsp;&nbsp;<input type="text" style="width: 80px" value="1" data-option="' + option + '" id="' + options[0] + '_' + options[6] + '_multiple">&nbsp;&nbsp;<button onclick="plusNum(this)">+</button></td>';
	...
	function reflushData() {
		...
		for(var i=0; i<3; i++) {
			...
			for(var j=0; j < data.length; j++) {
				...
				matchHtml += '<td style="width: 15%" id="' + match.id + '_胜_td" class="selectItem" data-id="' + match.id + '_胜" data-option="' + match.id + ';' +  match.home_team + ';' +  match.away_team+ ';' +  match.time_endsale + ';' +  match.current_spf.t3 + ';无;胜"><span id="team_name">' + match.home_team + '</span>&nbsp;&nbsp;<span style="color: red">' + match.current_spf.t3 + '</span></td>';
				matchHtml += '<td style="width: 8%" id="' + match.id + '_平_td" class="selectItem" data-id="' + match.id + '_平" data-option="' + match.id + ';' +  match.home_team + ';' +  match.away_team+ ';' +  match.time_endsale + ';' +  match.current_spf.t1 + ';无;平">' + match.current_spf.t1 + '</td>';
				matchHtml += '<td style="width: 15%" id="' + match.id + '_负_td" class="selectItem" data-id="' + match.id + '_负" data-option="' + match.id + ';' +  match.home_team + ';' +  match.away_team+ ';' +  match.time_endsale + ';' +  match.current_spf.t0 + ';无;负"><span style="color: red">' + match.current_spf.t0 + '</span>&nbsp;&nbsp;' + match.away_team + '</td>'; 
				matchHtml += '<td style="width: 15%" id="' + match.id + '_让球胜_td" class="selectItem" data-id="' + match.id + '_让球胜" data-option="' + match.id + ';' +  match.home_team + ';' +  match.away_team+ ';' +  match.time_endsale + ';' +  match.current_rqspf.t3 + ';' +  match.handicap + ';让球胜">' + match.home_team + '&nbsp;<span style="color: red">' + match.handicap + '</span>&nbsp;&nbsp;<span style="color: green">' + match.current_rqspf.t3 + '</span></td>';
				matchHtml += '<td style="width: 8%" id="' + match.id + '_让球平_td" class="selectItem" data-id="' + match.id + '_让球平" data-option="' + match.id + ';' +  match.home_team + ';' +  match.away_team+ ';' +  match.time_endsale + ';' +  match.current_rqspf.t1 + ';' +  match.handicap + ';让球平">' + match.current_rqspf.t1 + '</td>';
				matchHtml += '<td style="width: 15%" id="' + match.id + '_让球负_td" class="selectItem" data-id="' + match.id + '_让球负" data-option="' + match.id + ';' +  match.home_team + ';' +  match.away_team+ ';' +  match.time_endsale + ';' +  match.current_rqspf.t0 + ';' +  match.handicap + ';让球负"><span style="color: green">' + match.current_rqspf.t0 + '</span>&nbsp;&nbsp;' + match.away_team + '</td></tr>';
			}
...
<body>
	<div style="margin-left: auto; margin-right: auto; border: 1px solid red; width: 68%; height: 100%; overflow: auto;" align="center" id="main">
		...
		<div style="width: 100%;" id="football">
			...
			<div style="width: 100%" id="football_matchData">
		...
		<div style="border: 1px solid blue;width: 100%;min-height:300px;overflow: auto" id="bet_div">
			<span style="color: red;font-size: 20px">当前投注总额：<input type="text" style="width: 100px" id="totalCount" value="0">元 </span>&nbsp;&nbsp;<button onclick="betSubmit();">提交投注</button>
